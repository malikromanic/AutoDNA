...
def save_training_sample(npz_path, converted, labels, output_dir):
    ...
    # save each sensor's rgb image and shared time axis
    for sensor_name, result in converted.items():
        # shape (F, T, 3) uint8
        arrays[f'{sensor_name}_rgb'] = result['rgb_image']
        # save times and frequencies once per sensor (same for all if same fs)
    ...
    # convert labels to per-timestep label matrix
    # use times from first sensor (all sensors share same time axis)
    first_sensor = next(iter(converted.values()))
    times = first_sensor['times']
    y = labels_to_timeseries(labels, times)
    arrays['labels'] = y  # shape (T, n_classes) float32
...
def labels_to_timeseries(labels, times):
    ...
    for seg in labels:
        ...
        # find spectrogram columns that fall within this segment
